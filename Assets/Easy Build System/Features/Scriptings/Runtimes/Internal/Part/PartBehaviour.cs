...
namespace EasyBuildSystem.Runtimes.Internal.Part
{
    ...
    public class PartBehaviour : MonoBehaviour
    {
        ...
        public bool AvoidDestruction = false;
        ...
        public bool UnlessDestroy { get; set; }
        ...
        /// <summary>
        /// This method allows to check the allowed Build Surface below the part (Only work for Foundation type).
        /// </summary>
        public bool CheckBuildSurface()
        {
            if (!FreePlacement)
            {
                if (Type == PartType.Foundation)
                {
                    if (Physics.Raycast(transform.position + new Vector3(0, 0.1f, 0), Vector3.down, out RaycastHit Hit,
                        UseGroundUpper ? GroundUpperHeight : 0.3f, Physics.AllLayers))
                    {
                        ...
                        {
                            if (BuildManager.Instance.BuildingSupport == SupportType.VoxelandCollider)
                            {
                                if (Hit.collider.GetComponentInParent<VoxelandCollider>() == null)
                                    return true;
                            }
                            else if (BuildManager.Instance.BuildingSupport == SupportType.SurfaceCollider)
                            {
        ...
        /// </summary>
        public bool CheckElementsClipping()
        {
            Collider[] Colliders = PhysicExtension.GetNeighborsTypesByBox<Collider>(GetWorldPartMeshBounds().center,
                GetWorldPartMeshBounds().extents, transform.rotation, BuildManager.Instance.FreeLayers);
        ...
        /// </summary>
        public bool CheckElementsClippingOnSocket()
        {
            Collider[] Colliders = PhysicExtension.GetNeighborsTypesByBox<Collider>(GetWorldPartMeshBounds().center,
                GetWorldPartMeshBounds().extents, transform.rotation, BuildManager.Instance.FreeLayers);
            ...
            Colliders = PhysicExtension.GetNeighborsTypesByBox<Collider>(GetWorldPartMeshBounds().center,
                GetWorldPartMeshBounds().extents / 3.5f, transform.rotation, BuildManager.Instance.FreeLayers);
        ...
        {
            if (BuildManager.Instance.BuildingSupport == SupportType.AllCollider)
                return true;
            else if (BuildManager.Instance.BuildingSupport == SupportType.TerrainCollider)
                return collider.GetComponent<TerrainCollider>();
            else if (BuildManager.Instance.BuildingSupport == SupportType.VoxelandCollider)
